function [pred, necr, only_annotatns, modified_truth, tumor_im, notum_im, calc_width, calc_height, tot_width, tot_height] = get_modified_prediction(weight_file, mark_file, pred_file, tot_width, tot_height)

clusterPointsDisPos = 0.015;
clusterPointsDisNeg = 0.015;
...
modified_truth = uint8((iml >= lym_w) & (imn < nec_w));
only_annotatns = 127 * ones(size(modified_truth, 1), size(modified_truth, 2), 'uint8');
...
for iter = 1:length(m_type)
    ...
    if (strcmp(mt, 'LymPos'))
        for jter = 1:2:length(po)
            norm_x = po(jter);
            norm_y = po(jter+1);
            modified_truth = marking(modified_truth, 1,   mw, norm_x, norm_y, calc_width, calc_height, tot_width, tot_height);
            only_annotatns = marking(only_annotatns, 255, mw, norm_x, norm_y, calc_width, calc_height, tot_width, tot_height);
        end
    elseif (strcmp(mt, 'LymNeg'))
        for jter = 1:2:length(po)
            norm_x = po(jter);
            norm_y = po(jter+1);
            modified_truth = marking(modified_truth, 0, mw, norm_x, norm_y, calc_width, calc_height, tot_width, tot_height);
            only_annotatns = marking(only_annotatns, 0, mw, norm_x, norm_y, calc_width, calc_height, tot_width, tot_height);
        end
...
%[poly_x, poly_y] = deloop(poly_x, poly_y);

% sample 200 polygon vertices
