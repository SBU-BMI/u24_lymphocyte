...
if os.path.exists('model_large/X_train.pkl'):
    X_train = pickle.load(open('model_large/X_train.pkl', 'rb'));
else:
    X_train, mu, sigma = load_data();
    pickle.dump(X_train, open('model_large/X_train.pkl', 'w'));
    pickle.dump(mu, open('model_large/conv_mu.pkl', 'w'));
    pickle.dump(sigma, open('model_large/conv_sigma.pkl', 'w'));
...
for ep in range(100):
    for it in range(it_div):
        ...
        sys.stdout.flush();
        pickle.dump(ae, open("model_large/conv_ae_{}_{}.pkl".format(ep, it), 'w'));
        pickle.dump(ae, open('model_large/conv_ae.pkl', 'w'));
