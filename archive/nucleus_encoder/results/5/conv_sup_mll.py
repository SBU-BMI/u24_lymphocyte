...
char_target = T.imatrix('char_targets');
shape_target = T.ivector('shape_targets');
...
char_layer = layers.DenseLayer(incoming = fc_layer, num_units = 12, nonlinearity = sigmoid);
shape_layer = layers.DenseLayer(incoming = fc_layer, num_units = 7, nonlinearity = softmax);

char_pred = layers.get_output(char_layer);
char_loss = lasagne.objectives.binary_crossentropy(char_pred, char_target).mean();
shape_pred = layers.get_output(shape_layer);
shape_loss = lasagne.objectives.binary_crossentropy(shape_pred, shape_target).mean();
loss = (char_loss + shape_loss) / 2;

params = layers.get_all_params(char_layer, trainable=True) + shape_layer.get_params(trainable=True);
print params;
sys.stdout.flush();
updates = lasagne.updates.nesterov_momentum(loss, params, learning_rate=0.0005, momentum=0.975);

test_output = lasagne.layers.get_output(char_layer, deterministic=True);
