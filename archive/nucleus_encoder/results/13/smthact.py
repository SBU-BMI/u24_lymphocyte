...
class AgoLayer(lasagne.layers.Layer):
    def __init__(self, incoming, num_segs, W = lasagne.init.Normal(std = 0.2, mean = 0.0), P = lasagne.init.Normal(std = 2, mean = 0.0), **kwargs):
...
class HeLayer(lasagne.layers.Layer):
    def __init__(self, incoming, W = lasagne.init.Normal(std = 0.01, mean = -0.2), **kwargs):
        ...
        num_inputs = self.input_shape[1];
        self.W = self.add_param(W, (1, num_inputs), name = 'W', regularizable = True);
