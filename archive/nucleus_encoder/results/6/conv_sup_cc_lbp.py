...
def load_data(classn):
    ...
    for line in lines:
        ...
        if splitr < 0.2:
            ...
            f_test = np.concatenate((f_test, lbp[line_num]));
            y_test = np.concatenate((y_test, np.expand_dims(np.array(lab))));
        elif splitr >= 0.2 and splitr < 0.25:
            X_val = np.concatenate((X_val, png));
            f_val = np.concatenate((f_val, lbp[line_num]));
            y_val = np.concatenate((y_val, np.expand_dims(np.array(lab))));
        elif splitr >= 0.25:
            X_train = np.concatenate((X_train, png));
            f_train = np.concatenate((f_train, lbp[line_num]));
            y_train = np.concatenate((y_train, np.expand_dims(np.array(lab))));
        line_num += 1;
