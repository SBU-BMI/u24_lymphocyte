...
def val_fn_epoch(classn, val_fn, X_val, a_val, y_val):
    ...
    val_batches = 0;
    for batch in iterate_minibatches(X_val, a_val, y_val, batchsize = 200, shuffle = False):
...
def make_training_functions(network, encode_layer, input_var, aug_var, target_var):
    ...
    loss = lasagne.objectives.binary_crossentropy(output[:, :12], target_var[:, :12]).mean() + \
           lasagne.objectives.categorical_crossentropy(output[:, 12:], target_var[:, 12:]).mean();

    params = layers.get_all_params(network, trainable=True);
    updates = lasagne.updates.nesterov_momentum(loss, params, learning_rate=0.0001, momentum=0.975);
