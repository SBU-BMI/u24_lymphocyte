...
def load_data(classn):
    mu = pickle.load(open('model_large/conv_mu.pkl', 'rb'));
    sigma = pickle.load(open('model_large/conv_sigma.pkl', 'rb'));
    ...
    lines = [line.rstrip('\n') for line in open('../hand_segmentation/glioma/train/image/roundness.txt')];
    for line in lines:
        img = line.split('\t')[0];
        lab = [float(x) for x in line.split('\t')[1].split()];
        png = misc.imread('../hand_segmentation/glioma/train/image/' + img).transpose()[0 : 3, 9 : 41, 9 : 41];
    ...
    lines = [line.rstrip('\n') for line in open('../hand_segmentation/glioma/test/image/roundness.txt')];
    for line in lines:
        img = line.split('\t')[0];
        lab = [float(x) for x in line.split('\t')[1].split()];
        png = misc.imread('../hand_segmentation/glioma/test/image/' + img).transpose()[0 : 3, 9 : 41, 9 : 41];
        png = np.expand_dims(png, axis=0).astype(np.float32) / 255;
...
def synth_stack_pretrain_round(stack_train_fn, val_fn, classn):
...
def synth_pretrain_round(train_fn, val_fn, classn):
...
def train_round(train_fn, val_fn, classn, X_train, y_train, X_val, y_val):
    print("Starting training...");
    print("TrLoss\t\tVaLoss\t\tVaRMSE\t\tVaPear\t\tEpochs\t\tTime");
    num_epochs = 2000;
    batchsize = 100;
...
def split_validation(classn, weight_decay):
    ...
    synth_stack_pretrain_round(stack_train_fn, val_fn, classn);
    synth_pretrain_round(train_fn, val_fn, classn);

    #X_train, y_train, X_val, y_val, X_test, y_test = load_data(classn);
    #train_round(train_fn, val_fn, classn, X_train, y_train, X_val, y_val);
