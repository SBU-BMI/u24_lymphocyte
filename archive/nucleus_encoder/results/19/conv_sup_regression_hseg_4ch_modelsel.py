...
from smthact import SumLayer, SmthAct1Layer, SmthAct2Layer
...
def load_data(classn):
    mu = pickle.load(open('model_fullsize_nopool_4ch/conv_mu.pkl', 'rb'));
    sigma = pickle.load(open('model_fullsize_nopool_4ch/conv_sigma.pkl', 'rb'));

    X_test = np.empty(shape = (0, 4, 50, 50));
    X_val = np.empty(shape = (0, 4, 50, 50));
    X_train = np.empty(shape = (0, 4, 50, 50));

    y_test = np.empty(shape = (0, classn));
    y_val = np.empty(shape = (0, classn));
    y_train = np.empty(shape = (0, classn));

    lines = [line.rstrip('\n') for line in open('../hand_segmentation/glioma_from_yi/train/roundness.txt')];
    for line in lines:
        img = line.split('\t')[0];
        ...
        mask = misc.imread('../hand_segmentation/glioma_from_yi/train/mask/' + img).transpose();
        png = misc.imread('../hand_segmentation/glioma_from_yi/train/image/' + img).transpose();
    ...
    lines = [line.rstrip('\n') for line in open('../hand_segmentation/glioma_from_yi/test/roundness.txt')];
    for line in lines:
        img = line.split('\t')[0];
        lab = [float(x) for x in line.split('\t')[1].split()];
        if lab < 0:
        ...
        mask = misc.imread('../hand_segmentation/glioma_from_yi/test/mask/' + img).transpose();
        png = misc.imread('../hand_segmentation/glioma_from_yi/test/image/' + img).transpose();
        png_4ch = np.concatenate((png, np.expand_dims(np.array(mask), axis = 0)));
        png_4ch = np.expand_dims(png_4ch, axis=0).astype(np.float32) / 255;
...
def val_fn_epoch(classn, val_fn, X_val, a_val, y_val):
    val_err = 0;
    Er = np.empty(shape = (0, 200), dtype = np.float32);
...
def split_validation(classn, weight_decay, model_param):
    ...
    network_layers, input_var, aug_var, target_var, stack_params = build_network_from_ae(classn, model_param);
    val_fn, train_fn, stack_train_fn = make_training_functions(network_layers, input_var, aug_var, target_var, stack_params, weight_decay, model_param[4]);
...
def main(model_param):
    ...
    for v in range(split_n):
        Er, Or, Tr = split_validation(classn, weight_decay, model_param);
        save_result("regression_sp_{}".format(v), Er, Or, Tr);
        print("[CNN] RMSE: {:.4f} Pear: {:.4f}".format(comp_rmse(Or, Tr), comp_pear(Or, Tr)));


if __name__ == "__main__":
    model_param = (int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), float(sys.argv[5]));
