...
PS = 100;
cv_v = int(sys.argv[1]);
LearningRate = theano.shared(np.array(5e-4, dtype=np.float32));
BatchSize = 100;
...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/and_anne.txt')];
    valid_i = 0;
    for line in lines:
...
def train_round(num_epochs, network, valid_num, train_fn, val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test):
    ...
    for epoch in range(num_epochs+1):
        ...
        if epoch == 10:
            LearningRate.set_value(np.float32(0.33*LearningRate.get_value()));
        if epoch == 20:
            LearningRate.set_value(np.float32(0.33*LearningRate.get_value()));
        if epoch == 30:
            LearningRate.set_value(np.float32(0.33*LearningRate.get_value()));
