...
model_dump = 'model_vals/' + sys.argv[0].split('_deploy.py')[0] + '.py';
aug_fea_n = 1;
...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/train_on_nucleus_test_on_patch.txt')];
    valid_i = 0;
    for line in lines:
...
def save_visual_cases(X, Or, Tr):
    if not os.path.exists('./visual/' + sys.argv[0]):
        os.mkdir('./visual/' + sys.argv[0]);

    if not os.path.isfile('./visual/' + sys.argv[0] + '/truth.txt'):
        with open('./visual/' + sys.argv[0] + '/truth.txt', 'w'):
            pass;

    ind = sum(1 for line in open('./visual/' + sys.argv[0] + '/truth.txt'));
    for xi in range(X.shape[0]):
        ind += 1;
        im = X[xi].transpose().astype(np.uint8);
        misc.imsave('./visual/' + sys.argv[0] + '/case_{}.png'.format(ind), im);

    f = file('./visual/' + sys.argv[0] + '/output.txt', 'a');
    np.savetxt(f, Or, fmt='%.4f');
    f.close();

    f = file('./visual/' + sys.argv[0] + '/truth.txt', 'a');
