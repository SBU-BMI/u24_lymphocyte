...
APS = 300;
PS = 100;
POS_MAX_PROB = 0.19999;
NEG_MIN_PROB = 0.20001;
cv_v = int(sys.argv[1]);
...
def load_data_folder(classn, folder, is_train):
    ...
    for line in lines:
        ...
        if feedback:
            truth = float(line.split()[1]);
            prob = float(line.split()[-1]);
            if (truth > 0.5) and (prob >= POS_MAX_PROB):
                continue;
            if (truth < 0.5) and (prob <= NEG_MIN_PROB):
                continue;
...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/and_agree_luad10_brca10.txt')];
    valid_i = 0;
    for line in lines:
