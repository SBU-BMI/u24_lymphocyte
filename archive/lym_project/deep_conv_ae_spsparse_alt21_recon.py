...
BatchSize = 64;
filename_code = 21;
filename_model_ae = 'model_vals/deep_conv_autoencoder_spsparse_alt{}_model_{}.pkl'.format(filename_code, 6);
...
def build_autoencoder_network():
    ...
    nulay =            layers.Deconv2DLayer(layer, 3,    filter_size=(1,1), stride=1, crop='same', nonlinearity=identity);
    ...
    layer = layers.ElemwiseSumLayer([nulay, glblf]);
    ...
    nucl_var = lasagne.layers.get_output(nulay,    deterministic=True);
    glob_var = lasagne.layers.get_output(glblf,    deterministic=True);
    outp_var = lasagne.layers.get_output(network,  deterministic=True);

    return network, input_var, feat_var, mask_var, nucl_var, glob_var, outp_var;


def build_testing_function(network, input_var, feat_var, mask_var, nucl_var, glob_var, outp_var):
    print("building testing function");
    encode_decode_func = theano.function([input_var], [feat_var, mask_var, nucl_var, glob_var, outp_var]);
    print("finish building testing function");
    return encode_decode_func;


def save_visual(all_imag, all_feat, all_mask, all_nucl, all_glob, all_outp, mu, sigma):
    for xi in range(all_imag.shape[0]):
        ...
        nucl = np.clip(((all_nucl[xi].transpose() * sigma + mu) * 255.0), 0.1, 254.9).astype(np.uint8);
        glob = np.clip(((all_glob[xi].transpose() * sigma + mu) * 255.0), 0.1, 254.9).astype(np.uint8);
        ...
        misc.imsave("./visual_autoencoder/nucl_{}.png".format(xi), np.squeeze(nucl));
        misc.imsave("./visual_autoencoder/glob_{}.png".format(xi), np.squeeze(glob));
...
def main():
    ...
    # Build network
    network, input_var, feat_var, mask_var, nucl_var, glob_var, outp_var = build_autoencoder_network();
    encode_decode_func = build_testing_function(network, input_var, feat_var, mask_var, nucl_var, glob_var, outp_var);
    all_imag, all_feat, all_mask, all_nucl, all_glob, all_outp = exc_test(encode_decode_func, X_train);
    save_visual(all_imag, all_feat, all_mask, all_nucl, all_glob, all_outp, mu, sigma);
