...
from extensive_data_aug_260 import data_aug
from ch_inner_prod import ChInnerProd, ChInnerProdMerge, CenterCrop
...
PS = 260;
cv_v = int(sys.argv[1]);
LearningRate = theano.shared(np.array(1e-5, dtype=np.float32));
BatchSize = 14;
...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/adeno_new_test.txt')];
    valid_i = 0;
    for line in lines:
...
def build_network_from_ae(classn):
    ...
    add_0 = layers.Pool2DLayer(add_d, pool_size=(65,65), stride=65, mode='average_inc_pad');
    add_1 = batch_norm(layers.DenseLayer(add_0,  30, nonlinearity=leaky_rectify));
    add_e = layers.Pool2DLayer(CenterCrop(add_d, 20), pool_size=(25,25), stride=25, mode='average_inc_pad');
    add_f = batch_norm(layers.DenseLayer(add_e, 100, nonlinearity=leaky_rectify));
    ...
    cat_layer = lasagne.layers.ConcatLayer([add_1, add_f, add_4, aug_layer], axis=1);

    hdn_layer = layers.DenseLayer(cat_layer, 160, nonlinearity=leaky_rectify);
    hdn_layer = layers.DenseLayer(hdn_layer, 80,  nonlinearity=leaky_rectify);
    network = layers.DenseLayer(hdn_layer, classn, nonlinearity=sigmoid);
