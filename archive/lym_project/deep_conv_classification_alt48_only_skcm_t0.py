...
def load_data_folder(classn, folder, is_train):
    X = np.zeros(shape=(4000, 3, APS, APS), dtype=np.float32);
    y = np.zeros(shape=(4000, classn), dtype=np.int32);
...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/only_skcm.txt')];
    valid_i = 0;
    for line in lines:
...
def build_network_from_ae(classn):
    ...
    mask_map = SoftThresPerc(mask_rep, perc=98.0, alpha=0.1, beta=init.Constant(0.5), tight=100.0, name="mask_map");
