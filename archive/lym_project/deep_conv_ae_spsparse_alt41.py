...
def build_autoencoder_network():
    ...
    layer = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    layer = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    layer = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    layer = layers.Pool2DLayer(layer, pool_size=(2,2),stride=2, mode='max');
    layer = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    layer = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    prely = batch_norm(layers.Conv2DLayer(layer, 16,  filter_size=(3,3), stride=1, pad='same', nonlinearity=leaky_rectify));
    ...
    gllyr = batch_norm(layers.Conv2DLayer(glblf, 3,    filter_size=(1,1), nonlinearity=rectify), name="global_feature");

    glblf = batch_norm(layers.Deconv2DLayer(gllyr, 64, filter_size=(3,3), stride=1, crop='same', nonlinearity=leaky_rectify));
