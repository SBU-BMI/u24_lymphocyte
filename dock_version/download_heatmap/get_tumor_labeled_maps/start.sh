...
for files in ${MARKING_FOLDER}/*_weight.txt; do
    ...
    echo "${SVS} ${USER} ${WIDTH} ${HEIGHT} ${MARK}"

    run_get_tumor_pos_neg_map.sh /dev/MATLAB/MATLAB_Runtime/v94 ${SVS} ${USER} ${WIDTH} ${HEIGHT} ${MARK} ; \
    </dev/null
done

echo ${TUMOR_HEATMAPS_PATH}
echo ${TUMOR_IMAGES_TO_EXTRACT}
