...
from extensive_data_aug_200x200 import data_aug
...
def load_data():
    ...
    nbuf = 0;
    X_train = np.zeros(shape=(300000, 3, PS, PS), dtype=np.float32);
    lines = [line.rstrip('\n') for line in open('./data/vals/random_patches_for_all_svs_40X/label.txt')];
    for line in lines:
        full_path = './data/vals/random_patches_for_all_svs_40X/image_' + line.split()[0];
...
def build_autoencoder_network():
    ...
    featm    = batch_norm(layers.Conv2DLayer(prely, 320, filter_size=(1,1), nonlinearity=leaky_rectify));
    ...
    layer = batch_norm(layers.Deconv2DLayer(layer, 320, filter_size=(4,4), stride=2, crop=(1,1),  nonlinearity=leaky_rectify));
    ...
    glblf = batch_norm(layers.Deconv2DLayer(glblf, 32,  filter_size=(3,3), stride=1, crop='same', nonlinearity=leaky_rectify));
    glblf = batch_norm(layers.Deconv2DLayer(glblf, 24,  filter_size=(4,4), stride=2, crop=(1,1),  nonlinearity=leaky_rectify));
    glblf = batch_norm(layers.Deconv2DLayer(glblf, 24,  filter_size=(3,3), stride=1, crop='same', nonlinearity=leaky_rectify));
    glblf = batch_norm(layers.Deconv2DLayer(glblf, 24,  filter_size=(3,3), stride=1, crop='same', nonlinearity=leaky_rectify));
