...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/and_agree_luad10_skcm10_brca10_luad7_luad2.txt')];
    valid_i = 0;
    for line in lines:
...
def multi_win_during_val(val_fn, inputs, augs, targets):
    for idraw in [-1,]:
        for jdraw in [-1,]:
            inpt_multiwin = data_aug(inputs, mu, sigma);
...
def build_network_from_ae(classn):
    ...
    add_a = batch_norm(layers.Conv2DLayer(feat_map, 320, filter_size=(3,3), nonlinearity=leaky_rectify));
    add_b = batch_norm(layers.Conv2DLayer(add_a, 320, filter_size=(3,3), nonlinearity=leaky_rectify));
    add_c = batch_norm(layers.Conv2DLayer(add_b, 320, filter_size=(3,3), nonlinearity=leaky_rectify));
    add_d = batch_norm(layers.Conv2DLayer(add_c, 320, filter_size=(3,3), nonlinearity=leaky_rectify));
    add_0 = layers.Pool2DLayer(add_d, pool_size=(32,32), stride=32, mode='average_inc_pad');
    ...
    cat_layer = lasagne.layers.ConcatLayer([add_1, aug_layer], axis=1);
