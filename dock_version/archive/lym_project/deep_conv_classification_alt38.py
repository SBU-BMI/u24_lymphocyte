...
def load_data(classn, valid_num):
    ...
    lines = [line.rstrip('\n') for line in open(all_data_folder + '/nucleus_level_train_on_all.txt')];
    valid_i = 0;
    for line in lines:
...
def split_validation(classn, valid_num):
    ...
    train_round(32,  network, valid_num, new_params_train_fn, val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test);
