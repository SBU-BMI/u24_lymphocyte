...
def load_data():
    ...
    sys.stdout.flush();
    mu = np.mean(X_train[0::floor(X_train.shape[0] / 50000), :, :, :].flatten());
    sigma = np.std(X_train[0::floor(X_train.shape[0] / 50000), :, :, :].flatten());
...
for ep in range(100):
    for it in range(it_div):
        ...
        sys.stdout.flush();
        pickle.dump(ae, open("model_4ch/conv_ae_{}_{}.pkl".format(ep, it), 'w'));
        pickle.dump(ae, open('model_4ch/conv_ae.pkl', 'w'));
        pickle.dump(mu, open('model_4ch/conv_mu.pkl', 'w'));
        pickle.dump(sigma, open('model_4ch/conv_sigma.pkl', 'w'));
