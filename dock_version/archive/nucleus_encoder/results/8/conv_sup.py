...
def load_data(mu, sigma):
    ...
    for line in lines:
        ...
        if linen % 100 <= 19:
            ...
            y_test = np.concatenate((y_test, np.expand_dims(np.array([np.array(lab[12:]).argmax()]), axis=0)));
        elif linen % 100 >= 20 and linen % 100 <= 24:
            X_val = np.concatenate((X_val, png));
            y_val = np.concatenate((y_val, np.expand_dims(np.array([np.array(lab[12:]).argmax()]), axis=0)));
        elif linen % 100 >= 25:
            X_train = np.concatenate((X_train, png));
            y_train = np.concatenate((y_train, np.expand_dims(np.array([np.array(lab[12:]).argmax()]), axis=0)));
...
network = layers.DenseLayer(incoming = fc_layer, num_units = 7, nonlinearity = softmax);
...
for batch in iterate_minibatches(X_test, y_test, batchsize, shuffle=False):
    inputs, targets = batch;
    err, acc, pred = val_fn(inputs, targets);
    Pr = np.concatenate((Pr, np.array(pred)));
    Tr = np.concatenate((Tr, np.array(targets)));
    test_err += err;
    test_acc += acc;
    test_batches += 1;
print("Final results:");
