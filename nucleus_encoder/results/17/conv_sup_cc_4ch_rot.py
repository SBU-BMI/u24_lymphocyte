...
def load_data(classn):
    mu = pickle.load(open('model_4ch_rot/conv_mu.pkl', 'rb'));
    sigma = pickle.load(open('model_4ch_rot/conv_sigma.pkl', 'rb'));
    ...
    for line in lines:
        ...
        lab = [int(x) for x in line.split('\t')[1].split()][:classn];
...
def save_visual_cases(X, Or, Tr):
    ...
    for xi in range(X.shape[0]):
        ...
        misc.imsave("./visual/case_{}.png".format(ind), im[:, :, 0:3]);
...
def main():
    classes = ['Perinuclear halos', 'Gemistocyte', 'Nucleoli', 'Grooved', 'Clumped chromatin', 'Hyperchromasia',
               'Overlapping nuclei', 'Multinucleation', 'Severe anaplasia', 'Mitosis', 'Apoptosis', 'Other attribute', 'No nucleus'];
    classn = len(classes);
    split_n = 5;
