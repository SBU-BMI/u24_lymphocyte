...
from smthact import SmthActLayer
from smthact import DotLayer
...
def build_network_from_ae(classn):
    ...
    smth_act_layer = SmthActLayer(incoming = hidden_layer, x_start = -5, x_end = 5, num_segs = 10);
...
def make_training_functions(network, encode_layer, input_var, aug_var, target_var, stack_params, weight_decay):
    ...
    loss = lasagne.objectives.squared_error(output, target_var).mean() + \
           weight_decay * regularization.regularize_network_params(
                   layer = network, penalty = regularization.l2, tags={'small_weights' : True});
...
def split_validation(classn, weight_decay):
    ...
    network, encode_layer, input_var, aug_var, target_var, stack_params = build_network_from_ae(classn);
    val_fn, train_fn, stack_train_fn = make_training_functions(network, encode_layer, input_var, aug_var, target_var, stack_params, weight_decay);
