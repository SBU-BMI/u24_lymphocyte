...
trainset = list(all_rows[rands])        # shuffle the data
...
def load_data(dataset):
    i = 0
    for line in dataset:
        index = i//5000 # subfolder, each folder contains max 8000 images
        row = line.split()
        if '20X' in line:     # if this is 20X images
            vals = dest_folder + "/" + row[-1] + "_20X_" + str(index+1)
        else:
            vals = dest_folder + "/" + row[-1] + "_12X_" + str(index+1)
