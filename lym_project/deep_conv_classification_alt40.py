...
def load_data(classn, valid_num):
    ...
    sample_seed = (np.random.random_sample((X_train.shape[0],)) < 0.05);
    X_train = X_train[sample_seed, ...];
    y_train = y_train[sample_seed, ...];
...
def split_validation(classn, valid_num):
    ...
    train_round(51,  network, valid_num, new_params_train_fn, val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test);
    LearningRate.set_value(np.float32(0.10*LearningRate.get_value()));
